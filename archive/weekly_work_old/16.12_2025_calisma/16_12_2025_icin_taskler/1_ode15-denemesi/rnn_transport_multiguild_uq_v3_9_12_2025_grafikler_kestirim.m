...
% - Reactive transport simulation with advection, dispersion, and uncertainty quantification
  ...
  %%
    ...
    %% Fit mechanistic parameters and simulate trajectory (Ode15 değişimi yapıldı)
    p_fit = fit_mechanistic_params(t_exp, data_exp, x0);
    t = linspace(0, t_exp(end), 2000);
    ...
    % CHANGE: Switched to ode15s with stiff options
    options = odeset('RelTol', 1e-6, 'AbsTol', 1e-8, 'NonNegative', 1:10);
    [~, xTrain] = ode15s(@(t,y) trueODEfunc_multiguild(t,y,p_fit), t, x0, options);
    xTrain = xTrain'; % [10 x timeSteps]
...
function res = residuals_multiguild(p, t_exp, data_exp, x0)
    try
        % GÜNCELLENMİŞ SOLVER AYARLARI
        options = odeset('RelTol', 1e-3, 'AbsTol', 1e-6, 'NonNegative', 1:10);
        [t_sim, y_raw] = ode23s(@(t,y) trueODEfunc_multiguild(t,y,p), t_exp, x0, options);
        
        % Eğer solver zamanın sonuna kadar gidemeden patladıysa hata fırlat
        if t_sim(end) < t_exp(end) - 0.1
             error('Simulation stopped early');
        end

        y_sim = interp1(t_sim, y_raw, t_exp, 'linear', 'extrap'); % 'extrap' ekledik
        ...
        % Aşırı negatif değer kontrolü (Numerical noise protection)
        res(isnan(res)) = 100; % NaN varsa 100 ceza puanı yaz
        res = res(:);
        
    catch
        % Solver çökerse optimizasyonu durdurma, sadece o parametre setine "Kötü" de.
        % 1e3 yerine daha yönetilebilir bir ceza veriyoruz ki gradient hesaplanabilsin.
        res = 10 * ones(numel(data_exp), 1); 
    end
end
