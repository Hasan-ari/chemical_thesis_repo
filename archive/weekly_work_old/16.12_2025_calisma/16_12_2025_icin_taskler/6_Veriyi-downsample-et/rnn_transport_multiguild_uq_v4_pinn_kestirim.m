...
% - Reactive transport simulation with advection, dispersion, and uncertainty quantification
    ...
    %% 3. PINN Verisi ile Eğitim Seti Oluşturma (Hybrid Approach)
    disp('PINN verisi yükleniyor (High Resolution)...');
    loaded_data = load('PINN_Perfect_Data.mat'); 
    xTrain_raw = loaded_data.xTrain; % 2000 noktanın hepsini alıyoruz
    t = loaded_data.t;
    % -----------------------------------------
...
sequenceLength = 50; % number of past steps to use as input
...
% Define LSTM network (Derinleştirilmiş)
layers = [ ...
    sequenceInputLayer(size(xTrain,1))
    
    % Katman 1: Özellikleri çıkarır
    lstmLayer(128, 'OutputMode', 'sequence') 
    dropoutLayer(0.2)
    
    % Katman 2: Kararı verir
    lstmLayer(64, 'OutputMode', 'last')
    dropoutLayer(0.2)
    
    fullyConnectedLayer(size(xTrain,1))
    regressionLayer];

% Training options
options = trainingOptions('adam', 'MaxEpochs', 1000, 'MiniBatchSize', 512, 'InitialLearnRate', 0.01, 'Shuffle','every-epoch', ...
    'Verbose',true,'LearnRateDropPeriod', 400,'LearnRateSchedule', 'piecewise','ExecutionEnvironment', 'gpu');
...
% ---------------------------------------------------------
% 1. HAZIRLIK ve ZAMAN EKSENİ (DÜZELTİLMİŞ)
% ---------------------------------------------------------
% LSTM pencere boyutu (Bunu artırdık ki miyop olmasın)
seqLen = 50; 

% Tahmin edilecek adım sayısını 'xTrain' verisinden hesaplıyoruz
% (Toplam Veri Uzunluğu) - (İlk Pencere Boyutu)
total_steps = size(xTrain, 2);
num_predictions = total_steps - seqLen;

% Zaman eksenini (t_val) buna göre kesiyoruz
% t'nin sonundan 'num_predictions' kadar geriye gidip alıyoruz.
t_val = t(end - num_predictions + 1 : end);

disp(['Tahmin edilecek adım sayısı: ', num2str(num_predictions)]);

% Çizdirilecek Bileşen İsimleri
bilesenler = {'H2', 'CO2', 'CH4 (Metan)', 'H2S (Sülfür)', 'SO4'};

% ---------------------------------------------------------
% 2. Kestirim A: Birbirinden Bağımsız Tahmin (One-Step Ahead)
% ---------------------------------------------------------
% predict fonksiyonu için X_val (girdi) hücresini manuel hazırlayalım
% Böylece yukarıdaki X değişkenine bağımlı kalmayız.
X_val = {};
for i = 1:num_predictions
    % Her adımda: i'den başlayıp seqLen kadar veriyi al
    X_val{end+1} = xTrain(:, i : i + seqLen - 1);
end

% Tahmin yap
Y_independent = predict(net, X_val, 'MiniBatchSize', 1);
Y_independent = Y_independent'; % [DeğişkenSayısı x Zaman] formatına çevir
...
% Başlangıç Yemi: İlk 'seqLen' kadar normalize veri
...
for k = 1:num_predictions
    ...
    % c) Pencereyi kaydır
    % En eski veriyi at (sütun 1), yeni tahmini sona ekle
    current_input = [current_input(:, 2:end), next_step'];
end

% ---------------------------------------------------------
% 4. DENORMALİZASYON (Gerçek değerlere dönüş)
% ---------------------------------------------------------
% Normalize verileri (mu ve sig kullanarak) gerçek birimlere çeviriyoruz
% Formül: x_gerçek = (x_norm * sig) + mu
...
% xTrain'i de grafik için eski haline getir (Mavi çizgi için)
xTrain_plot = (xTrain .* sig) + mu;

% Boyut Güvenlik Kontrolü (Grafik hatasını önlemek için)
% Eğer tahmin boyutu ile t_val boyutu milimetrik uyuşmazsa kırpma yapar.
len_min = min([length(t_val), size(Y_independent,2), size(Y_chained,2)]);
t_val = t_val(1:len_min);
Y_independent = Y_independent(:, 1:len_min);
Y_chained = Y_chained(:, 1:len_min);
