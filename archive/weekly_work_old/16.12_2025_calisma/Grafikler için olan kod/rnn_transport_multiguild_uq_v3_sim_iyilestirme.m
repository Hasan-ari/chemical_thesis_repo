...
for i = 1:5 % İlk 5 bileşen (H2, CO2, CH4, H2S, SO4)
    ...
    plot(t, xTrain(i,:), 'b-', 'LineWidth', 1.5, 'DisplayName', 'Simülasyon ');
...
function p_fit = fit_mechanistic_params(t_exp, data_exp, x0)
    disp('Parametre optimizasyonu başlıyor (Multi-Start)...');

    % 1. Sınırlar (Bounds) - Biraz daha genişletelim
    % [k_meth, k_sulf, k_aceto, Y_m, Y_s, Y_a, KI_m, KI_s, KI_a, k_precip, H2S_sat, H2_thr, DG_thr]
    lb = [1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 0,    0,    0,   -100];
    ub = [50,   50,   50,   2,    2,    2,    50,   50,   50,   10,   5,    5,    0];
    
    % 2. Optimizasyon Ayarları
    options = optimoptions('lsqnonlin', ...
        'Display','iter', ... % Kalabalık yapmasın
        'MaxFunctionEvaluations', 5000, ...
        'StepTolerance', 1e-6, ...
        'FunctionTolerance', 1e-6);

    % 3. MULTI-START DÖNGÜSÜ (Farklı yerlerden başlatma)
    best_resnorm = inf; % En iyi hata puanı (Sonsuzla başlat)
    best_p = [];        % En iyi parametreler
    
    num_tries = 20; % 20 kere farklı yerden dene
    
    for i = 1:num_tries
        if i == 1
            % İlk deneme: Bizim mantıklı tahminimiz
            p0 = [2, 2, 1, 0.05, 0.05, 0.05, 0.5, 0.5, 0.5, 0.1, 0.1, 0.01, -15];
        else
            % Diğer denemeler: Sınırlar içinde RASTGELE sayılar
            % Formül: lb + (ub - lb) * rand()
            p0 = lb + (ub - lb) .* rand(size(lb));
        end
        
        try
            % Dedektifi çalıştır
            [p_curr, resnorm_curr] = lsqnonlin(@(p) residuals_multiguild(p, t_exp, data_exp, x0), p0, lb, ub, options);
            
            % Eğer bu deneme, eskilerden daha iyiyse kaydet
            if resnorm_curr < best_resnorm
                best_resnorm = resnorm_curr;
                best_p = p_curr;
                fprintf('Deneme %d: Yeni rekor! Hata = %.4f\n', i, best_resnorm);
            end
        catch
            % Hata verirse (çözerse) geç
        end
    end
    
    p_fit = best_p;
    disp('En iyi parametreler bulundu.');
end

% --- Residuals Fonksiyonunda Ağırlıkları Korumayı Unutmayın ---
function res = residuals_multiguild(p, t_exp, data_exp, x0)
    try
        [t_sim, y_raw] = ode45(@(t,y) trueODEfunc_multiguild(t,y,p), t_exp, x0);
        y_sim = interp1(t_sim, y_raw, t_exp, 'linear'); 
        ...
        % Log Scale + Weighting
        diff = log1p(y_sim(:,1:5)) - log1p(data_exp);
        
        % AĞIRLIKLAR (SO4 ve H2S'e torpil)
        weights = [1, 1, 1, 20, 10]; % SO4 ve H2S'i iyice artırdım
        ...
        res = ones(numel(data_exp)*5,1) * 1e6;
    end
end
