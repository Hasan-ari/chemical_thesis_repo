...
% sandstone @ 40 °C with effective Henry scale factors, H2S flux patch,
...
% adjust if needed
%%% (sandstone): use 40 °C base constants + scale factors (solubility variant Hscp=c/p)
...
raw = readmatrix('Muller_2024_H2_Sandstone_at_40C.txt');
t_exp       = raw(:,1);  % days
...
save('best_fit_params_sandstone_40C.mat','p_fit','env','y0');
...
fileID = fopen('sandstone_40C_inc_rates.dat','w');
...
fclose(fileID);
fprintf('Wrote sandstone_40C_inc_rates.dat (incl. Fe_pool and reaction rates)\n');

% -------------------- Plots --------------------
%% Figure 1: Gases & Aqueous - sandstone (40 °C)
species = {'nH2_g','nCO2_g','nCH4_g','nH2S_g','H2(aq)','CO2(aq)','SO4','FeS','Biomass','Acetate','HCO3','S_{aq}^{tot}','Fe_{pool}'};
figure('Name','Gases & Aqueous - sandstone (40 °C)');
