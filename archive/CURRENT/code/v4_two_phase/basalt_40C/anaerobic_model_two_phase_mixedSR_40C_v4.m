...
% Basalt @ 40 °C with effective Henry scale factors, H2S flux patch,
...
% PARAMETERS
% -------------------------------------------------------------------------
% y-state vector (14 species; inc Fe pool):
...
% adjust if needed
%%% (Basalt): use 40 °C base constants + scale factors (solubility variant Hscp=c/p)
Hcp_H2_base  = 0.78;  % H2  (40 °C) (mmol/L/atm)
Hcp_CO2_base = 34.0;  % CO2 (40 °C) (mmol/L/atm)
Hcp_H2S_base = 90.0;  % H2S (40 °C) (mmol/L/atm)
...
raw = readmatrix('Muller_2024_H2_Basalt_at_40C.txt');
t_exp       = raw(:,1);  % days
...
save('best_fit_params_Basalt_34C.mat','p_fit','env','y0');
...
fileID = fopen('Basalt_40C_inc_rates.dat','w');
...
fclose(fileID);
fprintf('Wrote Basalt_40C_inc_rates.dat (incl. Fe_pool and reaction rates)\n');

% -------------------- Plots --------------------
%% Figure 1: Gases & Aqueous - Basalt (40 °C)
species = {'nH2_g','nCO2_g','nCH4_g','nH2S_g','H2(aq)','CO2(aq)','SO4','FeS','Biomass','Acetate','HCO3','S_{aq}^{tot}','Fe_{pool}'};
figure('Name','Gases & Aqueous - Basalt (40 °C)');
