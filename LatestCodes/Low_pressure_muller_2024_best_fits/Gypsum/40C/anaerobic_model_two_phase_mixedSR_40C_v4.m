...
% SO4_sat_gyp=36.0, k_diss_gyp=0.12, kla_H2S higher (25 d?¹)

function anaerobic_model_two_phase_mixedSR_40C_v4
% Gypsum @ 40 °C with effective Henry scale factors, H2S flux patch,
...
% PARAMETERS
% -------------------------------------------------------------------------
% y-state vector (13 species; inc. Fe pool):
...
%   [23] k_diss_gyp:  dissolution rate (1/day) feeding SO4 buffer
...
%   env.Hcp_*_base   : Henry bases (mmol/L/atm) for H2, CO2, H2S @ 40 °C
...
%   env.SO4_sat_gyp  : -buffered sulfate level (mmol/L)
%
...
% Henry solubility constants Hscp (c/p) @ 40°C, mmol/L/atm
% adjust if needed
%%% (Gypsum): use 40 °C base constants + scale factors (solubility variant Hscp=c/p)
Hcp_H2_base  = 0.77;  % H2  (40 °C) (mmol/L/atm)
Hcp_CO2_base = 33.0;  % CO2 (40 °C) (mmol/L/atm)
Hcp_H2S_base = 85.0;  % H2S (40 °C) (mmol/L/atm)
...
raw = readmatrix('Muller_2024_H2_Gypsum_at_40C.txt');
t_exp       = raw(:,1);  % days
...
S_tot0   = 0.02;  % tiny sulfide seed to allow early H2S(g) appearance
...
p0 = [ 0.06, 0.08, 0.03, ...
       ...
       10.0, 10.0, 25.0, ...   
       0.01, 3.0, 0.7, ...
       0.12, 0.10 ...          %  dissolution timescale & competition gate
...
env.pH_fun      = pH_fun;
env.pKa_H2S     = 6.95;  % modest T-adjusted pKa1; tweak only if needed

% %%% PATCH equilibrium SO4 at 25 °C ~30–40 mM (was 15.0 mM)
env.SO4_sat_gyp = 40.0; % mM — matches experimental plateaus

%% -------------------- Fit (solve residuals at t_exp) --------------------
[p_fit,~,~,~,~,~,~] = lsqnonlin(@(p) residuals_full(p, t_exp, data_exp, y0, env), p0, lb, ub, fit_opts);
save('best_fit_params_Gypsum_40C.mat','p_fit','env','y0');
...
fileID = fopen('Gypsum_40C_inc_rates.dat','w');
...
fclose(fileID);
fprintf('Wrote Gypsum_40C_inc_rates.dat (incl. Fe_pool and reaction rates)\n');

% -------------------- Plots --------------------
%% Figure 1: Gases & Aqueous - Gypsum (40 °C)
species = {'nH2_g','nCO2_g','nCH4_g','nH2S_g','H2(aq)','CO2(aq)','SO4','FeS','Biomass','Acetate','HCO3','S_{aq}^{tot}','Fe_{pool}'};
figure('Name','Gases & Aqueous - Gypsum (40 °C)');
