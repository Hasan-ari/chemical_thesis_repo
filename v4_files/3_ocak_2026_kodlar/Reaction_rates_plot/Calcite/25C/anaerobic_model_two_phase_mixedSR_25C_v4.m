...
% Calcite @ 25 °C with effective Henry scale factors, H2S flux patch,
...
% adjust if needed
%%% (Calcite): use 25 °C base constants + scale factors (solubility variant Hscp=c/p)
Hcp_H2_base  = 0.77;  % H2  (25 °C) (mmol/L/atm)
Hcp_CO2_base = 33.0;  % CO2 (25 °C) (mmol/L/atm)
Hcp_H2S_base = 88.0;  % H2S (25 °C) (mmol/L/atm)
...
raw = readmatrix('Muller_2024_H2_Calcite_at_25C.txt');
t_exp       = raw(:,1);  % days
...
env.SO4_sat_gyp = 12.0; % mM — matches experimental plateaus

%% -------------------- Fit (solve residuals at t_exp) --------------------
[p_fit,~,~,~,~,~,~] = lsqnonlin(@(p) residuals_full(p, t_exp, data_exp, y0, env), p0, lb, ub, fit_opts);
save('best_fit_params_Calcite_25C.mat','p_fit','env','y0');
...
fileID = fopen('Calcite_25C_inc_rates.dat','w');
...
fclose(fileID);
fprintf('Wrote Calcite_25C_inc_rates.dat (incl. Fe_pool and reaction rates)\n');

% -------------------- Plots --------------------
%% Figure 1: Gases & Aqueous - Calcite (25 °C)
species = {'nH2_g','nCO2_g','nCH4_g','nH2S_g','H2(aq)','CO2(aq)','SO4','FeS','Biomass','Acetate','HCO3','S_{aq}^{tot}','Fe_{pool}'};
figure('Name','Gases & Aqueous - Calcite (25 °C)');
