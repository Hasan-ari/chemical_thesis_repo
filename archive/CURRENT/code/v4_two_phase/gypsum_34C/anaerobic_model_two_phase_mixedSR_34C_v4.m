...
function anaerobic_model_two_phase_mixedSR_34C_v4
% Gypsum @ 34 °C with effective Henry scale factors, H2S flux patch,
...
% Henry solubility constants Hscp (c/p) @ 34°C, mmol/L/atm
% adjust if needed
%%% (Gypsum): use 34 °C base constants + scale factors (solubility variant Hscp=c/p)
Hcp_H2_base  = 0.75;  % H2  (34 °C) (mmol/L/atm)
Hcp_CO2_base = 33.0;  % CO2 (34 °C) (mmol/L/atm)
Hcp_H2S_base = 85.0;  % H2S (34 °C) (mmol/L/atm)
...
raw = readmatrix('Muller_2024_H2_Gypsum_at_34C.txt');
t_exp       = raw(:,1);  % days
...
save('best_fit_params_Gypsum_34C.mat','p_fit','env','y0');
...
fileID = fopen('Gypsum_34C_inc_rates.dat','w');
...
fclose(fileID);
fprintf('Wrote Gypsum_34C_inc_rates.dat (incl. Fe_pool and reaction rates)\n');

% -------------------- Plots --------------------
%% Figure 1: Gases & Aqueous - Gypsum (34 °C)
species = {'nH2_g','nCO2_g','nCH4_g','nH2S_g','H2(aq)','CO2(aq)','SO4','FeS','Biomass','Acetate','HCO3','S_{aq}^{tot}','Fe_{pool}'};
figure('Name','Gases & Aqueous - Gypsum (34 °C)');
